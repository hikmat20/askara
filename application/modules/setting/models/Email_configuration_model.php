...
class Email_configuration_model extends CI_Model
{
    ...
    public function check_table_and_migrate()
    {
        ...
        // Jika tabel kosong, lakukan migrasi data dari tabel settings (existing config)
        ...
        if ($count == 0) {
            ...
            foreach ($smtp_data as $dt) {
                $existing[$dt->setting_name] = $dt->value;
            }

            if (!empty($existing['smtp_host']) && !empty($existing['smtp_user'])) {
