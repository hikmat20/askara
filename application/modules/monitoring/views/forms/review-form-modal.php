        ...
        <div class="card card-custom shadow-sm mb-3">
            ...
            <div class="card-body p-0">
                <?php if (!empty($form->file_name)) : ?>
                    <?php 
                    $file_path = base_url('directory/FORM/1/' . $form->file_name);
...
(function () {
    ...
    $(document).on('submit', '#form-review-form-modal', function (e) {
        ...
        // Validasi
        if (!status) {
            Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Pilih salah satu aksi review.' });
            return;
        }
        
        if (status === 'COR' && note === '') {
            $('#invalid-note-form').removeClass('d-none');
            Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Catatan wajib diisi jika aksi adalah Kembalikan.' });
            return;
        }
